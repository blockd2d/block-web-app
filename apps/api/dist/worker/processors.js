...
export async function processJob(client, job) {
    switch (job.type) {
...
async function processClusterGenerate(client, job) {
    ...
    if (!org_id || !cluster_set_id)
        throw new Error('missing org_id/cluster_set_id');
    const { data: set } = await client.from('cluster_sets').select('*').eq('org_id', org_id).eq('id', cluster_set_id).single();
    if (!set)
        throw new Error('cluster_set not found');
    ...
    let q = client
        ...
        .limit(200000);
    if (value_min != null)
        q = q.gte('value_estimate', value_min);
    if (value_max != null)
        q = q.lte('value_estimate', value_max);
    ...
    if ((exclude_dnk || only_unworked) && (props || []).length) {
        ...
        for (let i = 0; i < propIds.length; i += 5000) {
            ...
            for (const row of inter || []) {
                ...
                if (o === 'do_not_knock' || o === 'dnk')
                    dnk.add(row.property_id);
            }
        }
        let filtered = props || [];
        if (exclude_dnk)
            filtered = filtered.filter((p) => !dnk.has(p.id));
        if (only_unworked)
            filtered = filtered.filter((p) => !worked.has(p.id));
        props = filtered;
    }
    ...
    for (let i = 0; i < clusters.length; i++) {
        const c = clusters[i];
        let total_value = 0;
        for (const pid of c.memberPropertyIds)
            total_value += (valueById.get(pid) || 0);
...
async function processExportSales(client, job) {
    ...
    const { data: rows, error } = await client
        .from('sales_view')
        .select('id,rep_id,rep_name,address1,city,state,zip,price,service_type,sale_status,pipeline_status,customer_name,customer_phone,customer_email,created_at,updated_at')
        .eq('org_id', org_id)
        .limit(200000);
    if (error)
        throw new Error(error.message);
...
async function processExportAssignments(client, job) {
    ...
    let q = client
        ...
        .eq('org_id', org_id);
    if (cluster_set_id)
        q = q.eq('cluster_set_id', cluster_set_id);
...
async function processTwilioSendSms(client, job) {
    ...
    if (!org_id || !message_id)
        throw new Error('missing org_id/message_id');
