...
function jitter(base, scale) {
    return base + (Math.random() - 0.5) * scale;
}
function dayISO(d) {
    return d.toISOString().slice(0, 10);
}
function squareHull(centerLng, centerLat, sizeDeg) {
    const s = sizeDeg / 2;
    return {
        type: 'Polygon',
        coordinates: [
...
async function main() {
    ...
    if (propErr)
        throw propErr;
    const hendricksProps = (insertedProps || []).filter((p) => p.county_id === countyH.id);
    ...
    const clusterable = hendricksProps.filter((p) => !p.tags?.dnk);
    ...
    for (let c = 0; c < CLUSTERS; c++) {
        ...
        if (!slice.length)
            break;
        const centerLat = slice.reduce((a, p) => a + Number(p.lat), 0) / slice.length;
        const centerLng = slice.reduce((a, p) => a + Number(p.lng), 0) / slice.length;
        const totalPotential = slice.reduce((a, p) => a + Number(p.value_estimate || 0), 0);
    ...
    const jobIds = [];
    const jobs = soldTwo.map((saleId, idx) => {
    ...
    for (const saleId of soldTwo) {
        attachments.push({ org_id: org.id, sale_id: saleId, type: 'photo_before', storage_path: `seed/${saleId}/before-1.jpg` }, { org_id: org.id, sale_id: saleId, type: 'photo_before', storage_path: `seed/${saleId}/before-2.jpg` }, { org_id: org.id, sale_id: saleId, type: 'photo_after', storage_path: `seed/${saleId}/after-1.jpg` });
    }
