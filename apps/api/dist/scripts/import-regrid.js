...
if (!CSV_PATH)
    throw new Error("Missing CSV_PATH env var");
...
function headerToField(header) {
    ...
    if (["model", "enum", "datasource", "generator", "type"].includes(name))
        name = name + "Field";
    return name;
}
...
function parseDateOrNull(raw) {
    ...
    // YYYY/MM/DD HH:mm:ss(.sss)?(+|-)HH
    m = t.match(/^(\d{4})\/(\d{1,2})\/(\d{1,2})\s+(\d{1,2}):(\d{2}):(\d{2})(\.\d+)?([+-]\d{2})$/);
    if (m) {
        const Y = m[1];
...
async function main() {
    ...
    const parser = fs.createReadStream(abs).pipe(parse({
        columns: true,
        bom: true,
        relax_quotes: true,
    ...
    async function flushBatch() {
        ...
        batchBytes = 0;
        batchParams = 0;
        const res = await withRetries(() => prisma.regridParcelsHendricksIn.createMany({
            data: current,
            skipDuplicates: true,
        ...
        for await (const record of parser) {
            if (!colInfo) {
                ...
                colInfo = headers.map((h) => {
                    const kind = INT_HEADERS.has(h) ? "int" : FLOAT_HEADERS.has(h) ? "float" : DATE_HEADERS.has(h) ? "date" : "string";
...
//# sourceMappingURL=import-regrid.js.map