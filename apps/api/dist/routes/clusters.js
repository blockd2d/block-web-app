...
export async function clustersRoutes(app) {
    ...
    app.get('/', async (req, reply) => {
        ...
        if (!cluster_set_id)
            return reply.code(400).send({ error: 'cluster_set_id required' });
    ...
    app.post('/assign', async (req, reply) => {
        ...
        if (!cluster_id)
            return reply.code(400).send({ error: 'cluster_id required' });
        const service = createServiceClient();
    ...
    app.get('/:id', async (req, reply) => {
        ...
        if (ctx.role === 'rep') {
            ...
            if (!repId || cluster.assigned_rep_id !== repId)
                return reply.code(403).send({ error: 'Forbidden' });
        }
        if (ctx.role === 'labor') {
    ...
    app.get('/:id/inspector', async (req, reply) => {
        ...
        const { data: cp, error: cpErr } = await service
            ...
            .limit(50000);
        if (cpErr)
            return reply.code(400).send({ error: cpErr.message });
        ...
        if (property_ids.length > 0) {
            ...
            for (let i = 0; i < property_ids.length; i += 1000) {
                ...
                for (const it of interactions || []) {
                    if (!latestByProperty.has(it.property_id))
                        latestByProperty.set(it.property_id, it);
                }
            }
        ...
        const repDistances = (reps || [])
            .filter((r) => r.home_lat != null && r.home_lng != null)
            .map((r) => ({
            id: r.id,
            name: r.name,
        ...
        const assigned_rep_distance_km = cluster.assigned_rep_id
            ? repDistances.find((r) => r.id === cluster.assigned_rep_id)?.distance_km ?? null
            : null;
