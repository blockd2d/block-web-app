...
export async function laborRoutes(app) {
    ...
    app.get('/jobs', async (req, reply) => {
        ...
        if (ctx.role === 'labor') {
            const laborer_id = await getLaborerIdForProfile(service, ctx.org_id, ctx.profile_id);
            if (!laborer_id)
                return reply.code(403).send({ error: 'Laborer not provisioned' });
    ...
    app.get('/availability', async (req, reply) => {
        ...
        if (ctx.role === 'labor') {
            ...
            if (!laborer_id)
                return reply.code(400).send({ error: 'laborer_id required' });
        }
        const { data, error } = await service
    ...
    app.post('/time-off', async (req, reply) => {
        ...
        if (!start_at || !end_at)
            return reply.code(400).send({ error: 'start_at and end_at required' });
        const { data, error } = await service
            .from('labor_time_off')
    ...
    app.post('/jobs/:id/start', async (req, reply) => {
        ...
        if (ctx.role === 'labor') {
            ...
            if (!job || job.laborer_id !== laborer_id)
                return reply.code(403).send({ error: 'Forbidden' });
        }
        else
            requireManager(req);
