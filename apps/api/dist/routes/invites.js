...
import { requireRoles } from './_helpers';
...
export async function invitesRoutes(app) {
    ...
    app.post('/accept', async (req, reply) => {
        ...
        if (uErr || !created.user)
            return reply.code(400).send({ error: uErr?.message || 'Unable to create user' });
        ...
        if (sErr || !sessionData.session)
            return reply.code(400).send({ error: 'Created user but unable to sign in' });
