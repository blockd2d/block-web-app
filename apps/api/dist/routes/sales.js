...
async function assertSaleAccess(service, ctx, saleId) {
    if (ctx.role !== 'rep')
        return { ok: true, repId: null };
    const repId = await getRepIdForProfile(service, ctx.org_id, ctx.profile_id);
    if (!repId)
        return { ok: false, repId: null, error: 'Rep not provisioned' };
    ...
    if (!sale || sale.rep_id !== repId)
        return { ok: false, repId, error: 'Forbidden' };
...
export async function salesRoutes(app) {
    ...
    app.get('/', async (req, reply) => {
        ...
        if (ctx.role === 'rep') {
            ...
            if (!repId)
                return reply.code(403).send({ error: 'Rep not provisioned' });
        }
        // Date window
        let since = null;
        let until = null;
        ...
        if (statuses.length === 1)
            query = query.eq('pipeline_status', statuses[0]);
        if (statuses.length > 1)
            query = query.in('pipeline_status', statuses);
        // Search across common fields
        const search = q.q ? sanitizeLike(String(q.q)).slice(0, 120) : '';
        if (search) {
            // Supabase `.or()` expects a comma-delimited filter string.
            query = query.or(`customer_name.ilike.%${search}%,customer_phone.ilike.%${search}%,customer_email.ilike.%${search}%,address1.ilike.%${search}%`);
        }
    ...
    app.get('/:id/attachments', async (req, reply) => {
        ...
        if (!access.ok)
            return reply.code(access.error === 'Rep not provisioned' ? 403 : 403).send({ error: access.error });
        if (ctx.role === 'labor')
            return reply.code(403).send({ error: 'Forbidden' });
    ...
    app.post('/', async (req, reply) => {
        ...
        let rep_id = null;
        if (ctx.role === 'rep')
            rep_id = await getRepIdForProfile(service, ctx.org_id, ctx.profile_id);
        else
            rep_id = req.body?.rep_id || null;
        if (!rep_id)
            return reply.code(400).send({ error: 'rep_id required' });
    ...
    app.put('/:id', async (req, reply) => {
        ...
        for (const [k, v] of Object.entries(updates)) {
            const prev = before[k];
            if (prev !== v)
                changed[k] = { from: prev ?? null, to: v ?? null };
        }
        if (changed.status) {
    ...
    app.post('/:id/attachments', async (req, reply) => {
        ...
        const { data: row, error: iErr } = await service
            ...
            .single();
        if (iErr)
            return reply.code(400).send({ error: iErr.message });
    ...
    app.post('/:id/signature', async (req, reply) => {
        ...
        if (!dataUrl)
            return reply.code(400).send({ error: 'data_url required' });
        ...
        if (upErr)
            return reply.code(400).send({ error: upErr.message });
