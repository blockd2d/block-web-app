...
export async function clusterSetsRoutes(app) {
    ...
    app.post('/', async (req, reply) => {
        ...
        const { data: set, error } = await service
            ...
            .single();
        if (error || !set)
            return reply.code(400).send({ error: error?.message || 'failed' });
    ...
    app.get('/:id/suggest-assignments', async (req, reply) => {
        ...
        if (setErr || !set)
            return reply.code(404).send({ error: 'Cluster set not found' });
        // Fetch active reps
        ...
        if (repErr)
            return reply.code(400).send({ error: repErr.message });
        const { data: clusters, error: cErr } = await service
            ...
            .limit(20000);
        if (cErr)
            return reply.code(400).send({ error: cErr.message });
        const repsArr = reps || [];
        const clustersArr = clusters || [];
        if (repsArr.length === 0 || clustersArr.length === 0)
        ...
        for (const c of sorted) {
            let best = null;
            for (const r of repsArr) {
                if (r.home_lat == null || r.home_lng == null)
                    continue;
                ...
                if (!best || score < best.score)
                    best = { rep_id: r.id, distance_miles: d, score };
            }
            if (!best)
                continue;
    ...
    app.post('/:id/assign', async (req, reply) => {
        ...
        if (!Array.isArray(assignments) || assignments.length === 0)
            return reply.code(400).send({ error: 'assignments required' });
        const service = createServiceClient();
...
//# sourceMappingURL=cluster-sets.js.map