...
function clampRange(input: any): TimeRange {
  const r = String(input || 'week');
  if (r === 'month' || r === 'all') return r;
  return 'week';
}

export function getRangeWindow(rangeInput: any) {
  ...
  let priorSince: Date | null = null;
  let priorUntil: Date | null = null;
  let priorSinceDay: string | null = null;
  let priorUntilDay: string | null = null;
...
export function enumerateDaysISO(startDayISO: string, endDayISO: string) {
  const out: string[] = [];
